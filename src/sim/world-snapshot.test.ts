...
describe('world snapshot/restore', () => {
  it('round-trips the grid, funnels, tubes, sink tally, and tick', () => {
    ...
    const snapshot = captureWorldSnapshot(grid, funnels, tubes, sinkCounter, 42);
    ...
    const restored = restoreWorldSnapshot(grid, sinkCounter, snapshot);
  ...
  it('restored funnel/tube instances are independent copies, not aliases of the originals', () => {
    ...
    const snapshot = captureWorldSnapshot(grid, funnels, [], sinkCounter, 0);
    const restored = restoreWorldSnapshot(grid, sinkCounter, snapshot);
