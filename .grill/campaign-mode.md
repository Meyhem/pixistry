...
11. **The Vent shipped without a scenario that requires it** -- *decided, eyes open:* the tool is
    ...
    look used. The promising home is restoring the `purity` goals Phase 7 had to drop: those failed
    because a Sink inevitably drains bulk reagent into the purity denominator, and a Vent is exactly
    the disposal route that doesn't count (measured: with a Vent above the Sink, 3057 px of bulk went
    to the vent and only 98 px reached the sink tally, versus 3154 px with the Sink alone). Tuning
    that needs real browser playtesting, which was blocked on a concurrent UI refactor.
