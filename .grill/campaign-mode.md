...
| **8** 🚧 | Optional toys: vent, thermometer, pulsed funnel, catalyst pad, valve | **sim layer done for Vent + Catalyst Pad; UI surfaces pending.** Scoped by the row's own "as scenarios demand" rule: built the **Vent** (§6 point 12) and the **Catalyst Pad** (point 16), skipped **pulsed funnel** (point 14) and **valve** (point 15) because nothing on the ladder asks for either, and deferred the **thermometer** (point 13) with ladder #16 -- see Decision 10.<br><br>**Vent** -- deliberately *not* a parallel system to the Sink. `grid.sinkMask` values became a `SinkMaskValue` enum (`None`/`Sink`/`Vent`), so a cell is a sink or a vent but never both, by construction; `stepSinks` takes a second `SinkCounter` and dispatches by port kind; the same `paintSinkLine` message draws both, carrying a `port` field. New `'ventLimit'` Goal kind reads the vent tally rather than the sink tally -- a separate kind from `'limit'` rather than a flag on it, so an authoring site can't confuse "don't let this into your product" with "don't dump this much into the room". `SetupCommand`'s `'sink'` gained an optional `port`.<br><br>**Catalyst Pad** -- `grid.catalystStrength`, a per-cell whole-number multiplier (not one global constant, so a scenario can tune a bench the way `radiatorRadius`/`radiatorTargetK` already are per-cell). `react.ts`'s `tryReact` takes the stronger pad under the two reacting cells and multiplies the rule's per-tick `probability` by it, clamped to 1. Applied strictly *after* the `minTempK` check: catalysis buys rate, never ignition -- there's a test pinning that a 255x pad still can't start the Haber reaction at ambient temperature. New `'catalyst'` `SetupCommand` and `ToolKind`.<br><br>**Both toys were tuned on headless measurement, not guesses** -- the UI was mid-refactor in a concurrent session, so scenarios were driven directly against the real tick order (the `pixistry-debug` skill's own advice: prefer a vitest-style harness to driving the live browser). Two findings changed what shipped: (1) **the Catalyst Pad decisively fixes haber-plant.** Same sink layout, fixed 3600-tick window: no pad 33 NH3, 2x 53, 5x 76, 10x 118, against a goal of 100. `haber-plant` now grants the `'catalyst'` tool but does *not* pre-place a pad -- catalysis is that scenario's whole lesson, so painting it has to be the player's move -- and the goal stays reachable without one (par is 180s, three times the measured window), so the pad closes the "tedious -> satisfying" gap rather than being mandatory. (2) **the ladder's "vent <= 30 NO2" on copper-etch is a fake constraint, and was deliberately not wired.** Six rounds of painting Cu into the acid column, 600 ticks each, with and without a ceiling-wide Vent: 19 CuNO32Aq collected / 0 NO2 vented without, 18 / 11 with, and zero NO2 anywhere on the grid at the end either way. `NO2 + H2O -> HNO3(aq) + NO` (already in `reactions.ts`) re-absorbs the fumes into the water the etch reaction makes alongside them, so NO2 can never pile up in that open tank -- a limit of 30 is unfailable and venting buys nothing. Shipping it would be exactly the decorative goal Decision 9 warns against.<br><br>**Still to do:** the Catalyst Pad has no UI surface yet (spec handed to the concurrent UI-refactor session: Thermal & Mixing entry plus a 1-10 strength slider), so haber-plant currently grants a tool no player can pick up, and the pad's in-browser playtest is blocked behind that. The Vent's UI *is* in place and verified in the browser. Finding the Vent a scenario that genuinely demands it remains open -- the purity-goal theory was tested and refuted, see Decision 11. |
...
10. **Thermometer + ladder #16 deferred again** -- *decided:* the probe itself is small (it reuses the
    radiator overlay pattern), but the goal kind it exists to serve isn't. A useful temperature-band
    goal has to name *which* probe it's scoring, which means probes need per-instance identity and
    tracked state like funnels/tubes rather than a plain mask, plus a "held in band for N seconds"
    accumulator in `objectives.ts`. And a player-placed probe is a cheat vector -- nothing stops
    placing it somewhere trivially in-band -- so #16 would need its probes pre-placed by `setup`
    anyway. That's a coherent piece of work, but it's a phase's worth on its own rather than a
    Phase 8 "small optional toy", and it was the one Phase 8 item with no already-shipped scenario
    waiting on it.
11. **The Vent shipped without a scenario that requires it** -- *decided, eyes open:* the tool is
    built, tested and correct, but copper-etch turned out not to motivate it (see the Phase 8
    build-order note for the measurement), so no scenario currently sets a `ventLimit`. This is the
    same honest position Phase 6 took with the Zero Waste achievement -- wired and working, but not
    yet exercised by content -- rather than bolting an unfailable goal onto a level to make the tool
    look used.

    The obvious candidate home was restoring the `purity` goals Phase 7 dropped, on the theory that
    a Sink pollutes its own purity denominator with bulk reagent while a Vent is a disposal route
    that doesn't count. **Browser playtesting refuted that**, and it's worth writing down so nobody
    re-derives it. Playing photo-paper to 26/50 AgCl and reading the Sink's own tally gives: H2O
    2322, NaCl 301, AgNO3 300, AgNO3(aq) 76, NaCl(aq) 73, **AgCl 26**, NaNO3(aq) 26 -- total 3124,
    i.e. **0.83% purity** against a 90% target. Two things kill it, and a Vent fixes neither:
    - the pollution isn't only bulk water (74%); a fifth of it is *undissolved* NaCl/AgNO3 dropping
      straight through the pool to the floor before it ever gets a chance to dissolve and react;
    - dumping the grid shows AgCl (mean y 98.9), NaNO3(aq) (98.0), AgNO3(aq) (98.7) and NaCl(aq)
      all settling into **the same two rows** -- there is no vertical separation between product and
      waste to place a Vent into.

    A Vent is *species-blind* by construction (it eats whatever rests on it, exactly like a Sink),
    so it can only ever discriminate by position -- and here there is no position that separates
    them. The tool that discriminates by species is the Filter, and Phase 7 already found the Filter
    can't evict liquid that's already resident in a cell. So purity stays dropped, and finding the
    Vent a home is still open work rather than a solved follow-up.

Still open: Phase 8's two shipped toys need their UI surfaces before they're reachable by a player,
and the Vent needs a scenario. Nothing blocks a Phase 9.
